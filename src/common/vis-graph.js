...
class VisGraph extends mixinBehaviors([ItemViewBehavior], PolymerElement) {
  static get template() {
    return html`
      ...
      <token-ajax id="tokenAjaxDetails"
                  last-response="{{typeDetails}}"
                  _handleErrorResponse="{{handleError()}}"></token-ajax>

      <div id="visLayout"
           class="layout horizontal displayLength"
           style="flex-grow: 1">
      ...
      <paper-dialog id="visDialog" class="vis-dialog">
        ...
        <template is="dom-if" if="[[typeDetails.type]]">
          <props-table items="[[_attributes(typeDetails.type)]]"
                       title="Type"
                       with-row-stripes></props-table>
        </template>
  ...
  setData(data) {
    ...
    let container = this.$.vis_container;
    ...
    this.network.on('click', function (params) {
      self.handleSelectNode(this.getNodeAt(params.pointer.DOM));
    });
  ...
  enhanceNodeWithIcon(data, i) {
    ...
    if (!data.nodes[i].isQueridNode) {
      ...
      egeriaColor = getComputedStyle(document.documentElement)
                      .getPropertyValue('--egeria-primary-color');
    }

    let displayName = data.nodes[i].dispName === undefined ? ''
                                                           : data
                                                              .nodes[i]
                                                                .dispName;
    let groupInfo = data.nodes[i].groupInfo;
    let label = data.nodes[i].label;
    let icon = this.getIconAsSVG(data.nodes[i].group);
    ...
    const nodeComponentUrl =
      `data:image/svg+xml;charset=utf-8, ${encodeURIComponent(nodeComponent)}`;
  ...
  determineLegendData(data) {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (uniqueObjects[data[i].group] === undefined) {
        ...
        let { icon, groupColor } =
          this.getIconAndColor(currentNode, egeriaColor);
        ...
        uniqueObjects[data[i].group].appearances = uniqueObjects[data[i].group]
                                                    .appearances + 1;
      }
    }
  ...
  getIconAsSVG(group) {
    ...
    if (this.groups[group] === undefined
        || this.groups[group].icon === undefined) {
      return '';
    }
